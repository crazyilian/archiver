...
int main(int argc, char **argv) {
    ...
    try {
        if (args.IsOptionExists("h") || args.GetOptionsNumber() == 0) {  // src -h    // src
            PrintHelpMessage();
        } else if (args.GetOptionsNumber() != 1 || !args.GetValuesWithNoOption().empty()) {
            throw InvalidNumberOfArguments();
        } else if (args.IsOptionExists("c")) {  // src -c archive_name file1 [file2 ...]
            Compress(args.GetValuesByOption("c"));
        } else if (args.IsOptionExists("d")) {  // src -d archive_name
